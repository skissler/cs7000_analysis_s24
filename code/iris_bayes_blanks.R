...
iris_petallengths <- stan(
	file="code/iris_petallengths_blanks.stan",
	data=list(
		n=nrow(iris_ex1), 
...
# Generate a reduced dataset containing just the petal lengths: 
iris_ex2 <- iris %>% 
	filter(Structure=="Petal" & Metric=="____") %>% 
	mutate(SpeciesIndex=case_when(
		Species=="setosa"~1,
		Species=="versicolor"~2,
		Species=="virginica"~3))
...
iris_petalwidths <- stan(
	file="code/iris_petalwidths_blanks.stan",
	data=list(
		n=____, 
		n_species=____, 
		species=____,
...
iris_petalwidths_chains <- ____
iris_petalwidths_draws <- ____
iris_petalwidths_summaries <- ____
iris_petalwidths_lp <- ____

# Plot the ordered samples (chains) and histograms of the posterior distributions for the parameters: 
fig_iris_petalwidths_chains <- ____
fig_iris_petalwidths_hists <- ____

fig_postfit_iris_petalwidths <- ____
...
# Plot the posterior means as ellipses: --------------------------------------
...
# Plot the 95% prediction regions: --------------------------------------------
...
# Sample from the Stan model: 
iris_petalLW <- stan(
	file="code/iris_petalLW_blanks.stan",
	data=list(
		n=iris_ex3$n, 
...
# Generate credible ellipses for the posterior means: -----------------------
...
# Generate posterior ellipses for the prediction intervals: ------------------
